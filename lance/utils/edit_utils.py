...
from lance.utils import ptp_utils as ptp_utils
from lance.utils import seq_aligner as seq_aligner
...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store):
        ...
        if self.counter > self.start_blend:
            ...
            maps = [
                item.reshape(
                    self.alpha_layers.shape[0], -1, 1, 16, 16, self.MAX_NUM_WORDS
                )
                for item in maps
            ]
...
# Parent class for attention editing
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(
        ...
        self.mapper = seq_aligner.get_replacement_mapper(prompts, self.tokenizer).to(
            device
        )


# Second option: refinement
...
def get_equalizer(
    ...
    equalizer = torch.ones(len(texts), 77)
    # equalizer = torch.ones(1, 77)
    for ix, (word, val) in enumerate(zip(word_select, values)):
        if len(word.split(' ')) > 1:
            inds = []
            for w in word.split(' '):
                inds.extend(ptp_utils.get_word_inds(texts[ix], w, tokenizer))
        else:
            inds = ptp_utils.get_word_inds(texts[ix], word, tokenizer)

        equalizer[ix, inds] = val

        # inds = ptp_utils.get_word_inds(texts[0], word, tokenizer)
        # equalizer[:, inds] = val
    return equalizer
...
def make_controller(
    ...
    if blend_words is None:
        ...
        lb = LocalBlend(prompts, blend_words, tokenizer, device)
...
class NullInversion:
    """
    Class for null-text inversion: https://null-text-inversion.github.io/
    """
    ...
    def save_inversion(
        self, img_path, img, img_inv, x_t, uncond_embeddings, img_inv_dir
    ):
        """
        Store the inversion on disk
        args:
            img_path: path to the image
            img: the image
            img_inv: the inverted image
            x_t: the latent vector
            uncond_embeddings: the uncond_embeddings
            img_inv_dir: Directory to save to
        """
        # Save the image
        img_name = img_path.split("/")[-1].split(".")[0]
        path = os.path.join(img_inv_dir, img_name)
        os.makedirs(path, exist_ok=True)
        img.save(path + "/img.jpeg")
        # Save the inverted image
        img_inv.save(path + "/img_inv.jpeg")
        # Save the latent vector
        torch.save(x_t, path + "/x_t.pt")
        # Save the uncond_embeddings
        torch.save(uncond_embeddings, path + "/uncond_embeddings.pt")

    def load_inversion(self, img_path, img_inv_dir):
        """
        Load the inversion from disk
        args:
            img_path: path to the image
        returns:
            img: the image
        ...
        img_name = img_path.split("/")[-1].split(".")[0]
        path = os.path.join(img_inv_dir, img_name)
        img = plt.imread(os.path.join(path, "img.jpeg"))
        img_inv = plt.imread(os.path.join(path, "img_inv.jpeg"))
        # Load the latent vector
        x_t = torch.load(os.path.join(path, "x_t.pt"))
        # Load the uncond_embeddings
        uncond_embeddings = torch.load(os.path.join(path, "uncond_embeddings.pt"))
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        bar = tqdm(total=num_inner_steps * NUM_DDIM_STEPS, leave=True, disable=not accelerator.is_local_main_process)
        for i in range(NUM_DDIM_STEPS):
...
def text2image(
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:], mininterval=0, disable=not accelerator.is_local_main_process)):
